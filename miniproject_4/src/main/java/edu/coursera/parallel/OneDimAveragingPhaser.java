...
public final class OneDimAveragingPhaser {
    ...
    public static void runParallelFuzzyBarrier(final int iterations,
        ...
        // Different approaches for current task:
        // https://github.com/fjab76/project-gauss/blob/blog-phasers/src/main/scala/fjab/misc/OneDimensionalStencil.scala
        ...
        for (int ii = 0; ii < tasks; ii++) {
            ...
            threads[ii] = new Thread(() -> {
                ...
                for (int iter = 0; iter < iterations; iter++) {
                    ...
                    phs[i].arrive();
                    if (i - 1 >= 0) phs[i-1].awaitAdvance(iter);
                    if (i+1 < tasks) phs[i+1].awaitAdvance(iter);

                    // In my opinion here should be something like this
                    //   int currentPhase = phs[i].arrive();
                    //   if (i - 1 >= 0) phs[i-1].awaitAdvance(currentPhase);
                    //   if (i+1 < tasks) phs[i+1].awaitAdvance(currentPhase);
                    // In some reason this approach doesn't work
